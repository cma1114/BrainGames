...
namespace BrainGames.ViewModels
{
    public class DSViewModel : ViewModelBase
    {
        ...
        public void ResponseButton()
        {
            ...
            MasterUtilityModel.WriteDSGR(game_session_id, ++trialctr, spanlen, stimonms, stimoffms, Backward ? "b" : "f", String.Join("~", digitlist), repeats_set, repeats_cons, AutoIncrement, cor);
            ...
            if (AutoIncrement)
            {
                if (Backward)
                {
                    ...
                    {
                        if (MasterUtilityModel.RandomNumber(0, 3) <= 1) stimonms_b = Math.Max(stimonms_b - timedecms, mintimems);
                        else stimoffms_b = Math.Max(stimoffms_b - timedecms, mintimems);
                    }
                    else if (errtrialstreak_b > 0)
                    {
                        if (MasterUtilityModel.RandomNumber(0, 3) <= 1) stimonms_b = Math.Min(stimonms_b + timeincms, maxtimems);
                        else stimoffms_b = Math.Min(stimoffms_b + timeincms, maxtimems);
                    }
                }
                else
                {
